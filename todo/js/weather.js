...
const myGeoFn = (position) => {
	...
	//날시정보를 제공하는 오픈웨더맵의 API 서비스 - units=metric 옵션은 섭씨온도 설정
	...
	fetch(forecastUrl)
		...
		.then((info) => {
			...
			//사용자에게 필요한 날씨정보의 시간대를 09:00 ~ 21:00으로 가정
			let today9am = null; //오늘 오전 9시 예보를 저장할 변수 {dt: 1689498000, main: {…}, weather: Array(1), clouds: {…}, wind:
          ...
          2) 오늘날짜의 예보데이터인지 비교 확인
              (1) 예보데이터의 날짜를 기준으로 밀리세컨즈 변환 
              (2) 현재시간정보의 날짜를 기준으로 밀리세컨즈 변환
        */

				const forecastDate = new Date(forecast.dt_txt); //예보데이터의 날짜
				//console.log(`forcastDate = ${forecastDate}`);
				...
				//만약 같은 날(오늘)이면 4개의 예보를 저장할 변수에 각 시간에 해당하는 forecast를 할당
				if (forecastMill === todayDateMill) {
					if (forecastHour === 9) {
						today9am = forecast;
			...
			//
			let frsForecast; //첫번째 예보
			let secForecast; //두번째 예보

			const curHour = new Date().getHours();

			//현재 시간에 따라 위 두 예보변수에 값 할당
			if (curHour >= 12) {
				frsForecast = today6pm;
				secForecast = today9pm;
			} else {
				frsForecast = today9am;
				secForecast = today12pm;
			}

			//오늘 첫번째예보 출력
			const $firstFrame = document.querySelector('.today p:nth-of-type(1)>.frame');
			$firstFrame.style.backgroundImage = `url(https://openweathermap.org/img/wn/${frsForecast.weather[0].icon}@2x.png)`;
			//console.log('frsForecast=', frsForecast);

			$firstFrame.nextElementSibling.firstElementChild.textContent = `${new Date(
				frsForecast.dt_txt
			).getHours()}시`;
			$firstFrame.nextElementSibling.lastElementChild.textContent = frsForecast.main.temp;
	...
	fetch(forecastUrl)
		...
		.then((info) => {
			...
			//내일시간정보의 날짜를 밀리세컨즈로 변환
			const tomorrowMill = tomorrow.setHours(0, 0, 0, 0);
        ...
        */
				...
				const forecastMill = forecastDate.setHours(0, 0, 0, 0); //예보데이터의 날자를 밀리세컨즈로 ㅕㄴ환

				//만약 같은 날(내일)이면 2개의 예보를 저장할 변수에 각 시간에 해당하는 forecast를 할당
